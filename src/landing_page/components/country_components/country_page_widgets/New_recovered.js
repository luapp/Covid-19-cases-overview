import React from "react"
import "./Widgets_small.css"


function New_recovered ({new_recovered}) {
    return(
        <div className = "box-small-individual-country box-new-recovered">
            <div className = "title-h3-box">
                <h3 className = "title-h3">New recovered</h3>
            </div>
            <div className = "data-text-box-individual-country">
                <h2 className = "data-text-individual-country">+{new_recovered}</h2>
            </div>
        </div>
    )
}


export default New_recovered